...
@property (copy) CompletionBlock signupCompletionBlock;
@property (copy) CompletionBlock signupErrorBlock;
...
{
    self.signupCompletionBlock = successBlock;
    self.signupErrorBlock = errorBlock;

    [QBRequest createSessionWithSuccessBlock:^(QBResponse *response, QBASession *session) {
        [QBRequest signUp:user successBlock:^(QBResponse *response, QBUUser *user) {
            self.signupCompletionBlock(response, user);
        } errorBlock:^(QBResponse *response) {
            self.signupErrorBlock(response);
        }];    } errorBlock:^(QBResponse *response) {
            self.signupErrorBlock(response);
        }];
}
...
- (void)requestDialogsWithCompletionBlock:(void(^)())completionBlock errorBlock:(void (^)())errorBlock
{
    ...
    [QBRequest dialogsForPage:pageDialogs extendedRequest:nil
                 successBlock:^(QBResponse *response, NSArray *dialogObjects, NSSet *dialogsUsersIDs, QBResponsePage *pageResponseDialogs) {
...
//nala        [[SoundService instance] playNotificationSound];
    }
}
